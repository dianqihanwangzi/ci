...
def package_community = { arch ->
    ...
    wget -qnc http://fileserver.pingcap.net/download/builds/pingcap/tidb-plugins/optimization/v5.2.3/centos7/audit-1.so
    mv whitelist-1.so ${dst}
    mv audit-1.so ${dst}
    """
...
//         } else {
//             sh """
//             wget -qnc ${FILE_SERVER_URL}/download/builds/pingcap/${it}/test/${hashes[it]}/linux-amd64/${it}.tar.gz
//             tar -xzf ${it}.tar.gz
//             tar -czf ${it}-server-linux-${arch}.tar.gz -C bin/ \$(ls bin/)
//             """
//         }

//         sh """
//         tiup mirror publish ${it} ${VERSION} ${it}-server-linux-${arch}.tar.gz ${it}-server --arch ${arch} --os ${os} --desc="${descs[it]}"
//         """
//     }
